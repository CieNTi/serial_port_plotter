...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow),
      ...
      serialPort(nullptr), STATE(WAIT_START), NUMBER_OF_POINTS(500) {
  ui->setupUi(this);
...
void MainWindow::setupPlot() {
  ...
  ui->plot->axisRect()->insetLayout()->setInsetAlignment(0, Qt::AlignTop |
                                                                Qt::AlignRight);
}
/** ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  ...
  if (plotting) {
    ...
    for (i = 0; i < data_members; i++) {
      ...
      /* Rolling (v1.0.0 compatible) */
        ...
        /* Add data to Graph 0 */
        ui->plot->graph(channel)->addData(dataPointNumber,
                                          newData[channel].toDouble());
...
void MainWindow::on_actionRecord_PNG_triggered() {
  QString file_path = QFileDialog::getSaveFileName(
      this, tr("Save File"),
      QDir::homePath() + '/' + QDateTime::currentDateTime().toString("yyyyMMdd-hhmmss.zzz") + "-" +  QString::number(dataPointNumber) + ".png",
      tr("Images (*.png)"));

  //QMessageBox::information(this, "..", file_path);

 if (!file_path.isEmpty())
    ui->plot->savePng(file_path, 1920, 1080, 2, 50);
  // ui->plot->savePng (QDir::homePath() + '/' +
  // QString::number(dataPointNumber) + ".png", 1920, 1080, 2, 50);    // add
  // home path before picture's name ui->plot->savePng
  // (QString::number(dataPointNumber) + ".png", 1920, 1080, 2, 50);
}
/** ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
...
void MainWindow::on_actionConnect_triggered() {
  if (connected) {
    ...
    int dataBitsIndex =
        ui->comboData->currentIndex(); // Get index of data bits combo box
    int parityIndex =
        ui->comboParity->currentIndex(); // Get index of parity combo box
    int stopBitsIndex =
        ui->comboStop->currentIndex(); // Get index of stop bits combo box
    QSerialPort::DataBits dataBits;
    QSerialPort::Parity parity;
    QSerialPort::StopBits stopBits;
...
void MainWindow::openCsvFile(void) {
  m_csvFile =
      new QFile(QDir::homePath() + '/' +
                QDateTime::currentDateTime().toString("yyyy-MM-d-HH-mm-ss-") +
                "data-out.csv"); // add home path before picture's name
  // m_csvFile = new
  // QFile(QDateTime::currentDateTime().toString("yyyy-MM-d-HH-mm-ss-")+"data-out.csv");
  if (!m_csvFile)
    return;
